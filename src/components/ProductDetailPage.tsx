...
export default function ProductDetailPage() {
  ...
  // then generate from specs as last resort
  const generateSpecContent = () => {
    const lines: string[] = []
    lines.push(`${categoryEmoji} ${product.name}`)
    lines.push('')
    lines.push(`Category: ${product.category.name}`)
    if (product.steelGrade) lines.push(`Steel Grade: ${product.steelGrade}`)
    if (product.capacity) lines.push(`Capacity: ${product.capacity}`)
    if (product.dimensions) lines.push(`Dimensions: ${product.dimensions}`)
    if (product.leadTime) lines.push(`Lead Time: ${product.leadTime}`)
    lines.push(`Minimum Order Quantity: ${product.moq || 1} unit(s)`)
    lines.push('')
    lines.push('Contact us for detailed specifications and pricing.')
    return lines.join('\n')
  }

  const fullDetailsContent = product.longDescription || product.description || generateSpecContent()
