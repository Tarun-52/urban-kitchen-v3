...
import {
  ...
  emptyProductForm, emptyCategoryForm, emptyBlogForm,
  type ProductForm, type CategoryForm, type VariantForm, type QuotationItem, type BlogForm,
} from './admin/types'
...
import EmployeeLiveLocationTab from './admin/EmployeeLiveLocationTab'
...
export default function AdminDashboard() {
  ...
  const doFetchNotifications = useCallback(() => {
    ...
    fetch(`/api/notifications?userId=${userId}&limit=50`).then(r => r.json()).then(j => { if (j.status) { setNotifications(j.data.notifications || []); setUnreadCount(j.data.unreadCount || 0) } }).catch(console.error)
  }, [user])

  // ─── Effects ────────────────────────────────────────────
  useEffect(() => { fetch('/api/dashboard').then(r => r.json()).then(j => { if (j.status) setDashboardData(j.data) }).catch(console.error) }, [])
  useEffect(() => { doFetchNotifications() }, [doFetchNotifications])
  // Auto-refresh notifications every 30s
  useEffect(() => { const interval = setInterval(doFetchNotifications, 30000); return () => clearInterval(interval) }, [doFetchNotifications])
  ...
  useEffect(() => { if (adminTab === 'customers' || adminTab === 'users') doFetchUsers() }, [adminTab, doFetchUsers])
  ...
  // ─── Blog CRUD ──────────────────────────────────────────
  const doFetchBlogs = useCallback(() => {
    const params = new URLSearchParams()
    params.set('admin', 'true')
    params.set('limit', '50')
    if (blogStatusFilter !== 'all') params.set('status', blogStatusFilter)
    if (searchQueries.blog) params.set('search', searchQueries.blog)
    fetch(`/api/blog?${params}`).then(r => r.json()).then(j => { if (j.status) setBlogs(j.data.posts || []) }).catch(console.error)
  }, [blogStatusFilter, searchQueries.blog])

  const openNewBlog = () => { setEditBlog(null); setBlogForm(emptyBlogForm); setBlogDialog(true) }
  const openEditBlog = (b: any) => {
    setEditBlog(b)
    setBlogForm({
      title: b.title || '', slug: b.slug || '', excerpt: b.excerpt || '', content: b.content || '',
      featuredImage: b.featuredImage || '', category: b.category || '', categoryId: b.categoryId || b.categoryRef?.id || '',
      tags: b.tags || '', status: b.status || 'draft', featured: b.featured || false,
  ...
  const handleSaveBlog = async () => {
    try {
      ...
      const body = { ...blogForm, authorId: (user as any)?.id || null }
      const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const json = await res.json()
      if (json.status) {
        setBlogDialog(false); setEditBlog(null); doFetchBlogs()
        toast.success(editBlog ? 'Blog post updated' : 'Blog post created')
      } else { toast.error(json.message || 'Failed') }
    } catch (e) { console.error(e); toast.error('Failed to save blog post') }
  }
  ...
  switch (adminTab) {
    ...
    case 'leads': return <LeadsTab leads={leads} employees={employees} setLeadForm={setLeadForm} setLeadDialog={setLeadDialog} openLeadDetail={openLeadDetail} handleUpdateLeadStatus={handleUpdateLeadStatus} onLeadReassigned={doFetchLeads} />
    ...
    case 'users': return <UsersTab userList={userList} roleList={roleList} userDialog={userDialog} setUserDialog={setUserDialog} editUser={editUser} setEditUser={setEditUser} userForm={userForm} setUserForm={setUserForm} roleFilter={roleFilter} setRoleFilter={setRoleFilter} doFetchUsers={doFetchUsers} />
    ...
    case 'employee-live-location': return <EmployeeLiveLocationTab />
    ...
    case 'blog': return <BlogTab blogs={blogs} searchQuery={searchQueries.blog || ''} onSearchChange={(q) => handleSearch('blog', q)} statusFilter={blogStatusFilter} onStatusFilterChange={setBlogStatusFilter} openNew={openNewBlog} openEdit={openEditBlog} handleDelete={handleDeleteBlog} blogDialog={blogDialog} setBlogDialog={setBlogDialog} blogForm={blogForm} setBlogForm={setBlogForm} handleSaveBlog={handleSaveBlog} editBlog={editBlog} handleImageUpload={handleImageUpload} />
  ...
  const handleMarkAllRead = async () => {
    try {
      const userId = (user as any)?.id || ''
      await fetch('/api/notifications', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ markAllRead: true, userId }) })
      doFetchNotifications()
    } catch (e) { console.error(e) }
  }
  const handleNotifClick = (notif: any) => {
    ...
    if (notif.link) {
      setAdminTab(notif.link as AdminTab)
      setNotifOpen(false)
    }
  }
