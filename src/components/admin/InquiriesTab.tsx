...
export default function InquiriesTab({ inquiryList, setInquiryList }: InquiriesTabProps) {
  const updateInquiryStatus = async (id: string, status: string) => {
    try { await fetch(`/api/inquiries/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); setInquiryList(prev => prev.map(i => i.id === id ? { ...i, status } : i)); toast.success(`Inquiry marked as ${status}`) } catch (e) { toast.error('Failed') }
  }
