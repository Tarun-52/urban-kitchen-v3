  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-[#E8A33C]/10 border border-[#E8A33C]/30 flex items-center justify-center"><Icon className="w-5 h-5 text-[#E8A33C]" /></div>
        <div><h2 className="text-white text-xl font-bold">{title}</h2><p className="text-gray-500 text-xs">{description}</p></div>
      </div>
      {onAdd && <Button onClick={onAdd} className="bg-[#E8A33C] text-black hover:bg-[#E8A33C]/90 font-semibold"><Plus className="w-4 h-4 mr-2" />Add New</Button>}
    </div>
  )
}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Pipeline Value" value={fmt(pipelineValue)} icon={DollarSign} color="text-[#E8A33C]" delay={0.2} />
            ...
            <div className="space-y-2">
              {Object.entries(leads.reduce((acc: any, l: any) => { acc[l.source || 'unknown'] = (acc[l.source || 'unknown'] || 0) + 1; return acc }, {})).map(([src, cnt]: any) => (
                <div key={src} className="flex items-center justify-between py-2 border-b border-[#2a2a2a] last:border-0">
                  ...
                  <span className="text-[#E8A33C] font-semibold text-sm">{cnt}</span>
                </div>
              ))}
            </div>
          </CardContent>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Revenue" value={fmt(crud.items.reduce((a: number, c: any) => a + (c.revenue || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.3} />
      </div>
      ...
      <Dialog open={crud.dialog} onOpenChange={crud.setDialog}>
        <DialogContent className="bg-[#181818] border-[#2a2a2a] text-white max-w-lg">
          ...
          <DialogFooter><Button variant="ghost" onClick={() => crud.setDialog(false)} className="text-gray-400">Cancel</Button><Button onClick={crud.handleSave} className="bg-[#E8A33C] text-black hover:bg-[#E8A33C]/90 font-semibold">Save</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </motion.div>
  )
            ...
            <TableBody>{crud.items.map((p: any) => {
              ...
              return (
                <TableRow key={p.id} className="border-[#2a2a2a] hover:bg-white/5">
                  ...
                  <TableCell>{p.isDefault && <Badge className="bg-[#E8A33C]/10 text-[#E8A33C] border-[#E8A33C]/30 text-[10px]">Default</Badge>}</TableCell>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Pipeline Value" value={fmt(crud.items.filter((d: any) => d.status === 'open').reduce((a: number, d: any) => a + (d.value || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.2} />
      ...
      <div className="flex gap-3 overflow-x-auto pb-4">
        {grouped.map(({ stage, deals }) => (
          <div key={stage} className="min-w-[220px] flex-1">
            <div className="flex items-center justify-between mb-2"><h3 className="text-white text-sm font-semibold">{stage}</h3><Badge className="bg-[#E8A33C]/10 text-[#E8A33C] border-[#E8A33C]/30 text-[10px]">{deals.length}</Badge></div>
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              {deals.map((d: any) => (
                <Card key={d.id} className="bg-[#181818] border-[#2a2a2a] hover:border-[#E8A33C]/30 cursor-pointer transition-colors" onClick={() => crud.openEdit(d)}>
                  <CardContent className="p-3"><p className="text-white text-sm font-medium">{d.title}</p><p className="text-[#E8A33C] text-xs mt-1">{fmt(d.value || 0)}</p><p className="text-gray-500 text-xs mt-1">{d.probability}% probability</p></CardContent>
                </Card>
              ))}
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map(day => {
                ...
                return <div key={day} className={`min-h-[40px] p-1 rounded text-xs ${isToday ? 'bg-[#E8A33C]/10 border border-[#E8A33C]/30' : 'hover:bg-white/5'}`}><span className={`text-[10px] ${isToday ? 'text-[#E8A33C] font-bold' : 'text-gray-400'}`}>{day}</span>{dayEvents.slice(0, 2).map((e, i) => <div key={i} className={`text-[8px] truncate rounded px-1 ${e.type === 'task' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>{e.title}</div>)}</div>
              })}
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="WhatsApp" value={crud.items.filter((c: any) => c.channel === 'whatsapp').length} icon={MessagesSquare} color="text-[#E8A33C]" delay={0.3} />
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Cost" value={fmt(crud.items.reduce((a: number, s: any) => a + (s.cost || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.3} />
      </div>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {crud.items.map((intg: any) => (
          <Card key={intg.id} className="bg-[#181818] border-[#2a2a2a] hover:border-[#E8A33C]/30 transition-colors">
            <CardContent className="p-4">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Value" value={fmt(totalValue)} icon={DollarSign} color="text-[#E8A33C]" delay={0.1} />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Value" value={fmt(orders.reduce((a: number, o: any) => a + (o.total || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.3} />
      </div>
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {orders.map((o: any) => (
            <div key={o.id} className="flex items-center justify-between py-2 border-b border-[#2a2a2a] last:border-0">
              ...
              <div className="text-right"><p className="text-[#E8A33C] text-sm font-medium">{fmt(o.total)}</p><p className="text-gray-500 text-xs">{fmtDate(o.createdAt)}</p></div>
            </div>
          ))}
