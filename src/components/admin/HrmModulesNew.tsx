...
function ModuleHeader({ title, description, icon: Icon, onAdd }: { title: string; description: string; icon: React.ElementType; onAdd?: () => void }) {
  return <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[#E8A33C]/10 border border-[#E8A33C]/30 flex items-center justify-center"><Icon className="w-5 h-5 text-[#E8A33C]" /></div><div><h2 className="text-white text-xl font-bold">{title}</h2><p className="text-gray-500 text-xs">{description}</p></div></div>{onAdd && <Button onClick={onAdd} className="bg-[#E8A33C] text-black hover:bg-[#E8A33C]/90 font-semibold"><Plus className="w-4 h-4 mr-2" />Add New</Button>}</div>
}
          ...
          <div className="space-y-2">
            {Object.entries(employees.reduce((acc: any, e: any) => { acc[e.department] = (acc[e.department] || 0) + 1; return acc }, {})).map(([dept, cnt]: any) => (
              <div key={dept} className="flex items-center justify-between py-2 border-b border-[#2a2a2a] last:border-0">
                ...
                <div className="flex items-center gap-2"><div className="w-24 bg-[#0b0b0b] rounded-full h-2"><div className="bg-[#E8A33C] h-2 rounded-full" style={{ width: `${employees.length > 0 ? (cnt / employees.length * 100) : 0}%` }} /></div><span className="text-white text-sm font-medium w-8 text-right">{cnt}</span></div>
              </div>
            ))}
          </div>
        </CardContent></Card>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Budget" value={fmt(crud.items.reduce((a: number, d: any) => a + (d.budget || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.2} />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Avg Salary" value={fmt(crud.items.length > 0 ? crud.items.reduce((a: number, d: any) => a + (d.minSalary + d.maxSalary) / 2, 0) / crud.items.length : 0)} icon={DollarSign} color="text-[#E8A33C]" delay={0.2} />
...
function PayrollModule({ employees }: { employees: any[] }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Monthly Total" value={fmt(totalNet)} icon={Banknote} color="text-[#E8A33C]" />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total" value={fmt(items.reduce((a: number, s: any) => a + (s.netPay || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.1} />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Budget" value={fmt(crud.items.reduce((a: number, ap: any) => a + (ap.newSalary || 0), 0))} icon={DollarSign} color="text-[#E8A33C]" delay={0.3} />
      </div>
...
function SimpleCrudModule({ title, desc, icon: Icon, apiPath, defaultForm, statsConfig, tableConfig, formConfig }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      ))}</div><DialogFooter><Button variant="ghost" onClick={() => crud.setDialog(false)} className="text-gray-400">Cancel</Button><Button onClick={crud.handleSave} className="bg-[#E8A33C] text-black hover:bg-[#E8A33C]/90 font-semibold">Save</Button></DialogFooter></DialogContent></Dialog>
    </motion.div>
  )
}
...
function EmployeeWorkbookModule({ employees }: { employees: any[] }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Today's Entries" value={todayEntries.length} icon={CalendarDays} color="text-[#E8A33C]" />
      ...
      {/* Content */}
      {loading ? (
        <div className="flex items-center justify-center py-12"><div className="w-6 h-6 border-2 border-[#E8A33C]/30 border-t-[#E8A33C] rounded-full animate-spin" /></div>
          ...
          {Object.entries(byEmployee).sort(([a], [b]) => a.localeCompare(b)).map(([empName, empEntries]) => (
            <div key={empName}>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-8 h-8 rounded-full bg-[#E8A33C]/10 flex items-center justify-center">
              ...
              <div className="space-y-2 ml-4 pl-4 border-l border-[#2a2a2a]">
                {empEntries.map((entry: any) => (
                  <div key={entry.id} className={`bg-[#181818] border rounded-lg p-3 hover:border-[#E8A33C]/20 transition-colors ${entry.status === 'postponed' ? 'border-orange-500/20' : entry.workType === 'meeting' ? 'border-purple-500/10' : 'border-[#2a2a2a]'}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-xs text-gray-600">
                          ...
                          {entry.address && <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-[#E8A33C]" /><span className="truncate max-w-[150px]">{entry.address}</span></span>}
                        </div>
                        {entry.followUpDate && (
      ...
      <Dialog open={editDialog} onOpenChange={setEditDialog}>
        <DialogContent className="bg-[#181818] border-[#2a2a2a] text-white max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-[#E8A33C]">Edit Work Entry</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveEdit} disabled={saving} className="bg-[#E8A33C] text-black hover:bg-[#E8A33C]/90 font-semibold">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Admins" value={users.filter((u: any) => u.role?.roleName === 'admin').length} icon={Shield} color="text-[#E8A33C]" delay={0.2} />
                      ...
                      <TableCell className="max-w-[200px]">
                        <div className="flex flex-wrap gap-1">
                          {permList.length > 0 ? permList.slice(0, 4).map((p: string) => (
                            <span key={p} className="text-[10px] px-1.5 py-0.5 rounded bg-[#E8A33C]/10 text-[#E8A33C] border border-[#E8A33C]/20 capitalize">{p}</span>
      ...
      <Dialog open={editDialog} onOpenChange={setEditDialog}>
        <DialogContent className="bg-[#181818] border-[#2a2a2a] text-white max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ALL_PERMISSIONS.map(({ key, label }) => (
                    <button key={key} onClick={() => togglePermission(key)} type="button"
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium transition-all ${editPermissions.includes(key) ? 'bg-[#E8A33C]/10 border-[#E8A33C]/30 text-[#E8A33C]' : 'bg-[#0b0b0b] border-[#2a2a2a] text-gray-400 hover:border-[#E8A33C]/20'}`}>
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${editPermissions.includes(key) ? 'bg-[#E8A33C] border-[#E8A33C]' : 'border-gray-500'}`}>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setEditDialog(false)} className="text-gray-400">Cancel</Button>
            <Button onClick={handleSave} disabled={!!saving} className="bg-[#E8A33C] text-black hover:bg-[#E8A33C]/90 font-semibold">
...
export default function HrmModules({ adminTab, employees }: HrmModulesProps) {
  switch (adminTab) {
    ...
    case 'hrm-assets': return <SimpleCrudModule title="Assets" desc="Track company assets assigned to employees" icon={Monitor} apiPath="/api/assets" defaultForm={{ name: '', type: 'electronics', serialNo: '', value: 0, assignedTo: '', purchaseDate: '', status: 'available' }} statsConfig={(items: any[]) => [
      ...
      { label: 'Value', value: fmt(items.reduce((a: number, i: any) => a + (i.value || 0), 0)), icon: DollarSign, color: 'text-[#E8A33C]' },
    ...
    case 'hrm-documents': return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Verified" value={employees.filter((e: any) => e.documents).length} icon={Shield} color="text-[#E8A33C]" delay={0.3} />
        </div>
