...
export async function PUT(
  ...
  try {
    ...
    const { password, name, email, phone, roleId, status, canManageBlogs } = body
    ...
    if (canManageBlogs !== undefined) updateData.canManageBlogs = Boolean(canManageBlogs)
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { status: false, message: 'Failed to delete user' },
      { status: 500 }
    )
  }
}
