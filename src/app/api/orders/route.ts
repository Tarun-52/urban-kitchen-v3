...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await fetch('http://localhost:3000/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: `New Order #${order.orderNumber}`, message: `Order from ${customer?.name || 'Customer'} — Total: Rs. ${(order.total || 0).toLocaleString('en-IN')}`, type: 'order', link: 'orders', linkId: order.id }) })
