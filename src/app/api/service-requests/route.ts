...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await fetch('http://localhost:3000/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: `Service Request: ${serviceRequest.issue?.substring(0, 50) || 'New Request'}`, message: `Priority: ${serviceRequest.priority}. Status: ${serviceRequest.status}`, type: 'alert', link: 'service', linkId: serviceRequest.id }) })
