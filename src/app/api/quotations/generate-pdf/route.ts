...
class QuotationPDFBuilder {
  ...
  static async create(): Promise<QuotationPDFBuilder> {
    ...
    try {
      ...
      if (existsSync(logoPath)) {
        ...
        builder.logoImage = await pdfDoc.embedJpg(logoBytes)
      }
    } catch (e) {
  ...
  private drawCompanyHeader(settings: Record<string, string>) {
    ...
    const textStartX = this.margin + 15
    if (this.logoImage) {
      const logoSize = 55
      const logoX = this.margin + contentWidth - logoSize - 10
      const logoY = headerY + (headerBoxHeight - logoSize) / 2
