...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await fetch('http://localhost:3000/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: `New Quotation #${quotation.quotationNumber}`, message: `Quotation for ${quotation.customerName} — Rs. ${(quotation.amount || 0).toLocaleString('en-IN')}`, type: 'quotation', link: 'quotations', linkId: quotation.id }) })
