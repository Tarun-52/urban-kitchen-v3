...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const emp = leave.employee?.user?.name || 'Employee'
      await fetch('http://localhost:3000/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: `Leave Request: ${emp}`, message: `${leave.type} leave from ${new Date(leave.startDate).toLocaleDateString('en-IN')} to ${new Date(leave.endDate).toLocaleDateString('en-IN')}`, type: 'employee', link: 'leaves', linkId: leave.id }) })
