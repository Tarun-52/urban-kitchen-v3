...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await fetch('http://localhost:3000/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: `New Inquiry from ${inquiry.name}`, message: inquiry.subject || inquiry.message?.substring(0, 80) || 'General inquiry', type: 'lead', link: 'inquiries', linkId: inquiry.id }) })
